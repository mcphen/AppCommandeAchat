...
const uploadAndParse = async () => {
    ...
    try {
        const { data } = await axios.post(route('admin.articles.import.parse'), form, {
...
const confirmImport = async () => {
    ...
    try {
        const { data } = await axios.post(route('admin.articles.import.confirm'), {
...
<template>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-3 py-4 sm:px-6 sm:py-6">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2 self-start">
                    ...
                    <a
                        :href="route('admin.articles.template')"
    ...
    <!-- ─── Modal Import ─────────────────────────────────────────────────────── -->
    <Teleport to="body">
        <Transition name="imp-modal">
            <div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="closeImportModal" />
                ...
                <div class="relative z-10 w-full max-w-2xl rounded-2xl border bg-card shadow-2xl">
                    ...
                    <div class="flex items-center justify-between border-b px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-foreground">Importer des articles</h2>
                            ...
                            <div
                                ...
                                :class="
                                    importStep === s
                                        ...
                                        : (['upload', 'preview', 'result'].indexOf(importStep) > i)
                                          ? 'text-emerald-600 dark:text-emerald-400'
                                          : 'text-muted-foreground'
                                "
                            >
                                <CheckCircle2 v-if="['upload', 'preview', 'result'].indexOf(importStep) > i" class="h-3.5 w-3.5" />
                    ...
                    <div class="max-h-[60vh] overflow-y-auto px-6 py-5">
                        ...
                        <template v-if="importStep === 'upload'">
                            ...
                            <div class="mt-4 rounded-xl border bg-muted/20 p-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <span
                                        v-for="col in ['nom *', 'reference', 'categorie', 'description', 'unite', 'prix_unitaire', 'actif', 'nature']"
                                        :key="col"
                                ...
                                <p class="mt-2 text-xs text-muted-foreground">
                                    * Champ obligatoire. Si une référence existe déjà, la ligne sera mise à jour. Nature : achat (défaut) ou interne.
                                </p>
                            </div>
                        ...
                        <template v-if="importStep === 'preview' && parseResult">
                            <div class="mb-4">
                                <p class="text-sm text-muted-foreground">
                                    <span class="font-semibold text-foreground">{{ parseResult.total_rows }}</span>
                                    ligne{{ parseResult.total_rows > 1 ? 's' : '' }} détectée{{ parseResult.total_rows > 1 ? 's' : '' }}
                                ...
                                <table class="w-full text-xs">
                                    ...
                                    <tbody>
                                        ...
                                        >
                                            ...
                                            <td v-for="(cell, ci) in row.data" :key="ci" class="max-w-[120px] truncate px-3 py-2 text-foreground">{{ cell ?? '—' }}</td>
                                            <td class="px-3 py-2">
                        ...
                        <template v-if="importStep === 'result' && importResult">
                            <div class="flex flex-col items-center gap-4 py-4 text-center">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-muted-foreground">Les articles ont été importés avec succès.</p>
                                </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t px-6 py-4">
                        <template v-if="importStep === 'upload'">
                            <button @click="closeImportModal" class="rounded-xl border px-4 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted">
                                Annuler
                            </button>
                            <button
                        ...
                        <template v-if="importStep === 'preview'">
                            <button
                                @click="importStep = 'upload'; parseError = ''"
                                :disabled="confirming"
...
<style scoped>
.imp-modal-enter-active,
.imp-modal-leave-active {
    transition: opacity 0.2s ease;
}
.imp-modal-enter-from,
.imp-modal-leave-to {
    opacity: 0;
}
.imp-modal-enter-active .relative.z-10,
.imp-modal-leave-active .relative.z-10 {
    transition: transform 0.2s ease, opacity 0.2s ease;
}
.imp-modal-enter-from .relative.z-10,
.imp-modal-leave-to .relative.z-10 {
